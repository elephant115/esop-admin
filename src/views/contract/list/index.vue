    ...
    <el-dialog v-model="data.dialogVisible" :title="data.title" width="30%">
      ...
      >
        <el-form-item label="合约名称" prop="contractName">
          <el-input v-model="data.addForm.contractName" />
        </el-form-item>
        <el-form-item label="合约标题" prop="contractTitle">
          <el-input v-model="data.addForm.contractTitle" />
        </el-form-item>
        <el-form-item label="合约内容" prop="contractContext">
          <el-input v-model="data.addForm.contractContext" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import {
  ...
  adminDeleteById,
  adminSaveOrUpdate
} from '@/api/contract/contractList'
...
const addData = () => {
  formRef.value.validate((valid) => {
    ...
    adminSaveOrUpdate(data.addForm)
      .then((res) => {
        if (data.title === '新增') {
          ElMessage({
            type: 'success',
