<template>
  <div>
    <head-title tite="CB行权记录"></head-title>
    <div class="padding">
      <el-card>
        <el-row :gutter="20">
          <el-col :span="18">
            ...
            >
              ...
              <el-form-item label="行权类型">
                <el-select
                  v-model="data.form.type"
                  clearable
                  placeholder="请选择"
                >
                  <el-option label="RSU" :value="1" />
      ...
      <el-card class="mt-15">
        ...
        >
          ...
          <el-table-column
            ...
            label="申请人"
          />
          <el-table-column show-overflow-tooltip prop="type" label="行权类型">
            <template v-slot="{ row }">
              {{
                row.type == 1 ? 'RSU' : row.type == 2 ? '可以股票' : row.type
              }}
            </template>
          </el-table-column>
          <el-table-column
...
import { adminList } from '@/api/auditManagement/cbExerciseRecord'
const data = reactive({
  form: {
    pageNum: 1,
    pageSize: 10
